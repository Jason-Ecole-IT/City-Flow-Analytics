...
intersections.forEach(i => {
  ...
  i.marker = L.circleMarker([i.lat, i.lng], { radius: 15, color: trafficToColor(i.traffic) }).addTo(map);
});
...
function updateRoutes() {
  ...
  routes.forEach(r => {
    ...
    const polyline = L.polyline(r.coords, { color, weight: 6, opacity: 0.7 }).addTo(map);
...
function updateDashboard() {
  intersections.forEach(i => {
    const fill = document.getElementById(`gauge-${i.id}`);
    const text = document.getElementById(`gaugeText-${i.id}`);
    fill.style.width = i.traffic + "%";
    fill.style.background = i.marker.options.color;
    text.innerText = i.traffic + "%";
  });
}
...
intersections.forEach(i => {
  ...
  o1.value = i.id; o1.text = i.name; startSelect.appendChild(o1);
  const o2 = document.createElement("option");
  o2.value = i.id; o2.text = i.name; endSelect.appendChild(o2);
});

let currentRoutePolyline = null;

async function calculateRoute(startId, endId) {
  const start = intersections.find(i => i.id === startId);
  const end = intersections.find(i => i.id === endId);
  if (!start || !end) return;

  const url = `/route?startLng=${start.lng}&startLat=${start.lat}&endLng=${end.lng}&endLat=${end.lat}`;

  try {
    const resp = await fetch(url);
    if (!resp.ok) throw new Error("Erreur serveur ORS: " + resp.status);
    const data = await resp.json();

    const coords = data.features[0].geometry.coordinates.map(c => [c[1], c[0]]);

    if (currentRoutePolyline) map.removeLayer(currentRoutePolyline);
    currentRoutePolyline = L.polyline(coords, { color: "blue", weight: 6, opacity: 0.8, dashArray: "5,10" }).addTo(map);
    map.fitBounds(currentRoutePolyline.getBounds(), { padding: [50, 50] });
  } catch (err) {
    console.error("Erreur calcul ORS :", err);
  }
}
